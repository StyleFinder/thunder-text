...
export async function generateAds(
  ...
  console.log('🚀 Starting AIE ad generation pipeline...');
  ...
  try {
    ...
    console.log(`✅ Ad request created: ${adRequestId}`);
    ...
    if (params.imageUrl) {
      ...
      console.log(`✅ Image analyzed: ${category}/${imageAnalysis.subcategory}`);
    }
    ...
    console.log(`✅ Generated ${variantDrafts.length} ad variants`);
    ...
    const savedVariants = await Promise.all(
      rankedVariants.map(async (variant) => {
        ...
        if (error) {
          console.error(`Error saving variant ${variant.variant_number}:`, error);
          throw error;
        }
    ...
    console.error('❌ AIE pipeline error:', error);
