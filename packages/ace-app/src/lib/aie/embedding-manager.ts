...
export async function ensureBestPracticeEmbeddings(): Promise<{
  ...
  console.log('🔍 Checking best practice embeddings...');
  ...
  for (const practice of practices || []) {
    try {
      ...
      if (updateError) {
        console.error(`❌ Error updating practice ${practice.id}:`, updateError);
        errors++;
      } else {
        console.log(`✅ Generated embedding for: ${practice.title}`);
        generated++;
      }
      ...
      console.error(`❌ Error generating embedding for ${practice.title}:`, error);
      errors++;
    }
  }
