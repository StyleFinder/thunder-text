...
describe('Token Manager Integration', () => {
  ...
  describe('deactivateShopToken', () => {
    it('should soft delete shop token', async () => {
      ...
      const { deactivateShopToken } = await import(
  ...
  describe('Performance Tests', () => {
    it('should handle multiple concurrent requests', async () => {
      ...
      const promises = Array.from({ length: 10 }, () => getShopToken(TEST_SHOP))
    ...
    it('should cache tokens across multiple shops', async () => {
      ...
      const { storeShopToken, getShopToken } = await import(
        '@/lib/shopify/token-manager'
      )
      ...
      const start = Date.now()
      const results = await Promise.all(shops.map((shop) => getShopToken(shop)))
      const duration = Date.now() - start
