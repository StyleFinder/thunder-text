...
describe('Shopify Authentication', () => {
  ...
  describe('JWT Payload Validation', () => {
    ...
    it('should validate shop domain match', () => {
      ...
      // Extract shop from dest and compare
      const destShop = payload.dest.replace('https://', '')
      const matches = destShop.includes(requestShop.replace('.myshopify.com', ''))

      expect(matches).toBe(true)

      // Test mismatch
      const wrongShop = 'different-store.myshopify.com'
      const wrongDestShop = payload.dest.replace('https://', '')
      const doesNotMatch = !wrongDestShop.includes(
        wrongShop.replace('.myshopify.com', '')
      )
  ...
  describe('Token Format Validation', () => {
    ...
    it('should reject malformed tokens', () => {
      ...
      malformedTokens.forEach(({ token, reason }) => {
