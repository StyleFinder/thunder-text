...
export default function ShopSizes({ shop, onToast }: ShopSizesProps) {
  ...
  const fetchSizes = useCallback(async () => {
    try {
      ...
      logger.error("Error fetching sizes:", error as Error, {
        component: "ShopSizes",
      });
      onToast("Failed to load sizing sets", true);
    } finally {
  ...
  const handleUpdate = async (id: string) => {
    ...
    const isFallback =
      targetSize?.source === "fallback" || !targetSize?.store_id;
  ...
  const handleDelete = async (
    id: string,
    isTemplateOverride: boolean = false,
  ) => {
    const targetSize = sizes.find((size) => size.id === id);
    const isFallback =
      targetSize?.source === "fallback" || !targetSize?.store_id;
    ...
    if (isFallback) {
      onToast(
        "Default sizing templates cannot be deleted. Create a custom sizing set instead.",
        true,
      );
      return;
    }
    ...
    try {
      ...
      logger.error("Error deleting size:", error as Error, {
        component: "ShopSizes",
      });
          ...
          <div style={{ padding: "24px" }}>
            ...
            >
              ...
              >
                ...
                >
                  Supports letter sizes (XS-XXL), numeric (0-20), shoe sizes
                  (5-11), or custom text
                </p>
              </div>
                    ...
                    >
                      ...
                      {isFallback && (
                        ...
                        >
                          ...
                          >
                            Saving creates a custom copy for your shop so you
                            can edit the built-in template.
                          </p>
                        </div>
                      )}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="space-y-3 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <h3 className="text-lg font-semibold text-oxford-navy">
                            {size.name}
                          </h3>
                          {size.is_default && (
                            <Badge
                              variant="default"
                              className="bg-smart-blue-500"
                            >
                              DEFAULT
                            </Badge>
                          )}
                      ...
                      <div className="flex gap-2 ml-4">
                        ...
                        {!isFallback && (
                          <Button
                            ...
                            onClick={() =>
                              handleDelete(size.id, isTemplateOverride(size))
                            }
                          >
