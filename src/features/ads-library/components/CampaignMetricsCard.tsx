...
export default function CampaignMetricsCard({
  ...
  const totalPurchaseValue = insights.reduce(
    (sum, i) => sum + i.purchase_value,
    0,
  );
  const _totalPurchases = insights.reduce((sum, i) => sum + i.purchases, 0);

  // Weighted average conversion rate and ROAS
  const avgConversionRate =
    insights.length > 0
      ? insights.reduce((sum, i) => sum + i.conversion_rate, 0) /
        insights.length
      : 0;
  ...
  const conversionColor =
    avgConversionRate >= CONVERSION_BENCHMARK
      ? "text-green-600 border-green-600"
      : "text-red-600 border-red-600";
  const roasColor =
    overallROAS >= ROAS_BENCHMARK
      ? "text-green-600 border-green-600"
      : "text-red-600 border-red-600";
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {/* Conversion Rate */}
          <div
            className={`border-3 ${conversionColor} rounded-lg p-4 bg-white space-y-2`}
          >
          ...
          {/* ROAS */}
          <div
            className={`border-3 ${roasColor} rounded-lg p-4 bg-white space-y-2`}
          >
            <p className="text-sm font-semibold text-foreground">
          ...
          <div className="border-3 border-gray-300 rounded-lg p-4 bg-white space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              {insights.length} active campaign
              {insights.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {insights.map((insight) => {
              const campConvColor =
                insight.conversion_rate >= CONVERSION_BENCHMARK
                  ? "bg-green-600"
                  : "bg-red-600";
              const campRoasColor =
                insight.roas >= ROAS_BENCHMARK ? "bg-green-600" : "bg-red-600";
                ...
                >
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    <div className="flex items-center gap-1.5">
                      <div
                        className={`w-2 h-2 rounded-full ${campConvColor}`}
                      ></div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <div
                        className={`w-2 h-2 rounded-full ${campRoasColor}`}
                      ></div>
                      <span className="text-xs text-foreground">
