...
export default function CampaignSelector({
  ...
  const fetchCampaigns = async (adAccountId: string) => {
    try {
      ...
      // Fetch all campaigns (ACTIVE and PAUSED) - users often pause campaigns temporarily
      const response = await fetch(
        `/api/facebook/campaigns?shop=${shop}&ad_account_id=${adAccountId}`,
      );
      const data = await response.json();
      ...
      <div className="space-y-4">
        ...
        <Alert>
          ...
          <AlertDescription>
            ...
            <p className="text-sm">
              No Facebook ad accounts were found for your connected account.
              Please ensure you have access to at least one ad account in
            ...
            {loadingCampaigns ? (
              ...
              <Alert>
                ...
                <AlertDescription>
                  ...
                  <p className="text-sm">
                    No campaigns found for this ad account. Please create a
                    campaign in Facebook Ads Manager before continuing.
                  </p>
                </AlertDescription>
              </Alert>
            ) : (
            ...
            <div className="space-y-1">
              <p className="text-xs text-foreground">
                ...
                <span className="font-medium">
                  {formatBudget(selectedCampaign.daily_budget)}
                </span>
              </p>
              {selectedCampaign.lifetime_budget && (
                <p className="text-xs text-foreground">
                  Lifetime Budget:{" "}
                  <span className="font-medium">
                    {formatBudget(selectedCampaign.lifetime_budget)}
                  </span>
                </p>
              )}
            </div>
