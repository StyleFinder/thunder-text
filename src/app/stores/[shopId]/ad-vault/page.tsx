...
export default function AdVaultPage() {
  ...
  const { shopId: _shopId, isLoading: shopLoading } = useShop();
  ...
  useEffect(() => {
    if (
      ...
      (session && session.user.role !== "coach")
    ) {
  ...
  const fetchAdVault = async () => {
    try {
      ...
      logger.error("[Ad Vault] Error fetching ads:", error as Error, {
        component: "ad-vault",
      });
    } finally {
      setLoading(false);
  ...
  if (status === "loading" || loading || shopLoading) {
    return (
        ...
        >
          ...
          <div style={{ minWidth: "250px" }}>
            ...
            <input
              ...
              value={scoreRange[1]}
              onChange={(e) =>
                setScoreRange([scoreRange[0], parseFloat(e.target.value)])
              }
              style={{ width: "100%" }}
            />
          </div>
          ...
          <>
            <Table
              ...
              renderCell={(column, row) => {
                const storeName =
                  row.ad_request?.shop?.display_name ||
            ...
            <Card>
              ...
              >
                ...
                <div>
                  ...
                  <Text>
                    {selectedAd.ad_request?.platform} -{" "}
                    {selectedAd.ad_request?.goal}
                  </Text>
                </div>
                ...
                <div>
                  ...
                  <Badge
                    variant={
                      selectedAd.predicted_score >= 7
                        ? "success"
                        : selectedAd.predicted_score >= 5
                          ? "warning"
                          : "error"
                    }
                  >
