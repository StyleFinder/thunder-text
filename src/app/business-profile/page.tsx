...
const QUICK_ANSWER_OPTIONS: Record<string, { label: string; value: string }[]> = {
  discount_comfort: [
    { label: 'Rarely discount', value: 'I rarely discount my products and prefer to maintain full price positioning. I focus on communicating value and quality rather than competing on price, as I believe discounting can devalue my brand.' },
    { label: 'Sometimes discount', value: 'I occasionally run sales and promotions, maybe a few times per season or for special occasions like holidays and end-of-season clearances. I try to be strategic about when and how much I discount.' },
    { label: 'Frequently discount', value: 'I frequently run promotions and sales throughout the year. Discounts are a regular part of my marketing strategy to move inventory, attract new customers, and keep my audience engaged with fresh offers.' },
  ],
  inventory_size: [
    { label: 'Small (under 100)', value: 'I have a small, carefully curated inventory with fewer than 100 SKUs total. I focus on quality over quantity and prefer to offer a thoughtfully selected collection that my customers will love.' },
    { label: 'Medium (100-500)', value: 'I have a medium-sized inventory with around 100 to 500 products in my store. This gives me a nice balance of variety for my customers without being overwhelming to manage.' },
    { label: 'Large (500+)', value: 'I have a large inventory with over 500 products available in my store. Wide selection is important to my business model, as I want customers to find exactly what they are looking for.' },
  ],
  time_availability: [
    { label: 'Very limited (1-2 hrs/day)', value: 'I have very limited time and can only dedicate about 1-2 hours per day to my boutique. It is a side hustle alongside my other responsibilities, so I need to be efficient with my time.' },
    { label: 'Moderate (3-5 hrs/day)', value: 'I have moderate availability and spend about 3-5 hours daily on my boutique business. It is a significant part of my day but not quite full-time yet, so I balance it with other commitments.' },
    { label: 'Flexible (6+ hrs/day)', value: 'I have a flexible schedule and dedicate 6 or more hours daily to my boutique. It is my primary focus and main source of income, so I have plenty of time to implement new strategies and ideas.' },
  ],
  quarterly_goal: [
    { label: 'Increase sales', value: 'My main goal this quarter is to increase overall sales revenue and grow my customer base significantly. I want to focus on acquiring new customers while also encouraging repeat purchases from my existing loyal customers.' },
    { label: 'Clear inventory', value: 'My primary focus this quarter is clearing out slow-moving inventory and making room for fresh new products. I need to move older stock to free up cash flow and shelf space for upcoming seasonal items.' },
    { label: 'Build brand awareness', value: 'This quarter I want to focus on building brand awareness and establishing a stronger presence in my market. I want more people to know about my boutique and what makes it special and unique.' },
    { label: 'Improve margins', value: 'My goal this quarter is to improve profit margins by optimizing my pricing strategy, being more selective about discounting, and potentially finding better suppliers or negotiating better wholesale rates.' },
  ],
...
export default function BrandVoicePage() {
  ...
  const startInterview = async (mode: 'full' | 'quick_start' = 'full') => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setTotalQuestions(data.data.total_questions || (mode === 'quick_start' ? 12 : 21));
  ...
  const summarizePolicy = async (type: 'return' | 'shipping') => {
    ...
    try {
      ...
      setError('Please enter a valid URL (e.g., https://yourstore.com/policies/...)');
      return;
    }

    const setLoading = type === 'return' ? setIsSummarizingReturn : setIsSummarizingShipping;
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      logger.error(`Failed to summarize ${type} policy:`, error as Error, { component: 'business-profile' });
      setError(`Failed to summarize ${type} policy. Please try again or enter details manually.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const wordCount = (() => {
    if (currentPrompt?.prompt_key === 'policies_summary') {
      const parts: string[] = [];
      if (returnPolicyUrl.trim()) parts.push(`Return Policy URL: ${returnPolicyUrl.trim()}`);
      if (shippingPolicyUrl.trim()) parts.push(`Shipping Policy URL: ${shippingPolicyUrl.trim()}`);
      if (currentInput.trim()) parts.push(`Policy Details: ${currentInput.trim()}`);
      const combined = parts.join(' ');
      return combined.trim().split(/\s+/).filter((w) => w.length > 0).length;
    }
    return currentInput.trim().split(/\s+/).filter((w) => w.length > 0).length;
  })();
      ...
      <div className="w-full flex flex-col items-center" style={{ padding: '32px', background: '#fafaf9', minHeight: '100vh' }}>
        <div className="w-full" style={{ maxWidth: '800px' }}>
          <div style={{ marginBottom: '32px' }}>
            ...
            <p style={{ fontSize: '14px', color: '#6b7280', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>Your business profile has been created</p>
          </div>

          <Card style={{ background: '#ffffff', border: '2px solid #22c55e', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.08)' }}>
            <CardContent style={{ padding: '32px' }}>
              <div className="flex flex-col items-center" style={{ gap: '20px', textAlign: 'center' }}>
                <div style={{ background: '#dcfce7', borderRadius: '50%', padding: '16px' }}>
                  <CheckCircle className="h-12 w-12" style={{ color: '#22c55e' }} />
                </div>
                <div>
                  <h3 style={{ fontSize: '24px', fontWeight: 700, color: '#003366', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', marginBottom: '8px' }}>
                    Business Profile Complete
                  </h3>
                  <p style={{ fontSize: '14px', color: '#6b7280', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', lineHeight: 1.6 }}>
                    Your AI-powered business profile is ready and will guide all AI-generated content to match your unique brand voice.
                  </p>
                </div>
                {showProfileSuccess && (
                  <Alert style={{ background: '#dcfce7', border: '1px solid #86efac', borderRadius: '8px', padding: '12px', width: '100%' }}>
                    <CheckCircle className="h-4 w-4" style={{ color: '#22c55e' }} />
                    <AlertDescription style={{ fontSize: '14px', color: '#166534', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
                      Profile regenerated successfully! AI Coaches profile data has been updated.
                    </AlertDescription>
                  </Alert>
                )}
                {error && (
                  <Alert variant="destructive" style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '8px', padding: '12px', width: '100%' }}>
                ...
                <div className="flex flex-col w-full" style={{ gap: '12px', marginTop: '8px' }}>
                  ...
                  <Button
                    variant="outline"
                    onClick={() => router.push(buildUrl('/brand-voice/settings'))}
                    className="w-full"
                    style={{
                      background: '#ffffff',
                      color: '#0066cc',
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (response.ok && data.success && data.data?.profile) {
                          ...
                          logger.error('Failed to regenerate profile:', new Error(errorMessage), { component: 'business-profile' });
                        }
                      } catch (error) {
                        setError('Failed to regenerate profile. Please try again.');
                        logger.error('Failed to regenerate profile:', error as Error, { component: 'business-profile' });
                      } finally {
        ...
        <Dialog open={showResetConfirm} onOpenChange={setShowResetConfirm}>
          <DialogContent style={{ borderRadius: '8px' }}>
            <DialogHeader>
              <DialogTitle style={{ fontSize: '18px', fontWeight: 600, color: '#003366', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>Start Over?</DialogTitle>
              <DialogDescription style={{ fontSize: '14px', color: '#6b7280', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
                This will delete all your interview answers and generated profile. You'll need to complete the interview again.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter style={{ gap: '12px' }}>
              <Button
                ...
                style={{ borderRadius: '8px', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}
              >
                Cancel
              </Button>
              <Button
                onClick={async () => {
                  ...
                  try {
                    ...
                    logger.error('Failed to reset:', error as Error, { component: 'business-profile' });
                  } finally {
                    setIsResetting(false);
                  }
                }}
            ...
            <CardContent style={{ padding: '24px' }}>
              <div style={{ minHeight: '400px', maxHeight: '500px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {messages.map((msg, index) => {
                  ...
                  const isLastAIMessage = msg.type === 'ai' && index === messages.length - 1;
                  const quickOptions = msg.prompt_key ? QUICK_ANSWER_OPTIONS[msg.prompt_key] : null;
                  const showQuickAnswers = isLastAIMessage && quickOptions && currentPrompt?.prompt_key === msg.prompt_key;
                      ...
                      >
                        ...
                        {showQuickAnswers && (
                          <div style={{ marginTop: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            <p style={{ fontSize: '12px', color: '#6b7280', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', marginBottom: '4px' }}>
            ...
            <Card style={{ background: '#ffffff', border: '2px solid #0066cc', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.08)' }}>
              <CardContent style={{ padding: '24px' }}>
                <div className="flex flex-col items-center" style={{ gap: '16px', textAlign: 'center' }}>
                  ...
                  >
                    {isGeneratingProfile && <RefreshCw className="h-5 w-5 animate-spin" />}
                    {isGeneratingProfile ? 'Generating Your Profile...' : 'Generate Business Profile'}
                  </Button>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent style={{ padding: '32px' }}>
                <div className="flex flex-col items-center" style={{ gap: '20px', textAlign: 'center' }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '14px', color: '#6b7280', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', lineHeight: 1.6 }}>
                      Your AI-powered business profile is ready. It will guide all AI-generated content to match your unique brand voice and messaging.
                    </p>
                  </div>
                  <div className="flex flex-col w-full" style={{ gap: '12px', marginTop: '8px' }}>
                    <Button
                ...
                {currentPrompt.prompt_key === 'policies_summary' ? (
                  <>
                    <div style={{ marginBottom: '20px' }}>
                      <Label htmlFor="return-policy-url" style={{ fontSize: '14px', fontWeight: 600, color: '#003366', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', display: 'block', marginBottom: '8px' }}>
                        Return Policy URL <span style={{ color: '#6b7280', fontWeight: 400 }}>(optional)</span>
                      </Label>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <Input
                          ...
                          style={{ flex: 1, padding: '12px', fontSize: '14px', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', border: '1px solid #e5e7eb', borderRadius: '8px', background: '#ffffff', color: '#001429' }}
                        />
                        <Button
                          type="button"
                          onClick={() => summarizePolicy('return')}
                          disabled={isSubmitting || isSummarizingReturn || !returnPolicyUrl.trim()}
                          style={{
                            background: isSubmitting || isSummarizingReturn || !returnPolicyUrl.trim() ? '#e5e7eb' : '#0066cc',
                            color: '#ffffff',
                            borderRadius: '8px',
                            padding: '12px 16px',
                            fontSize: '14px',
                            ...
                            fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',
                            border: 'none',
                            cursor: isSubmitting || isSummarizingReturn || !returnPolicyUrl.trim() ? 'not-allowed' : 'pointer',
                            whiteSpace: 'nowrap',
                            display: 'flex',
                            alignItems: 'center',
                            gap: '6px',
                        ...
                        >
                          {isSummarizingReturn && <RefreshCw className="h-4 w-4 animate-spin" />}
                          {isSummarizingReturn ? 'Summarizing...' : 'Summarize'}
                        </Button>
                      </div>
                    </div>
                    <div style={{ marginBottom: '20px' }}>
                      <Label htmlFor="shipping-policy-url" style={{ fontSize: '14px', fontWeight: 600, color: '#003366', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', display: 'block', marginBottom: '8px' }}>
                        Shipping Policy URL <span style={{ color: '#6b7280', fontWeight: 400 }}>(optional)</span>
                      </Label>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <Input
                        ...
                        <Button
                          ...
                          disabled={isSubmitting || isSummarizingShipping || !shippingPolicyUrl.trim()}
                          style={{
                            background: isSubmitting || isSummarizingShipping || !shippingPolicyUrl.trim() ? '#e5e7eb' : '#0066cc',
                            color: '#ffffff',
                            borderRadius: '8px',
                            padding: '12px 16px',
                            fontSize: '14px',
                            ...
                            fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',
                            border: 'none',
                            cursor: isSubmitting || isSummarizingShipping || !shippingPolicyUrl.trim() ? 'not-allowed' : 'pointer',
                            whiteSpace: 'nowrap',
                            display: 'flex',
                            alignItems: 'center',
                            gap: '6px',
                        ...
                        >
                          {isSummarizingShipping && <RefreshCw className="h-4 w-4 animate-spin" />}
                          {isSummarizingShipping ? 'Summarizing...' : 'Summarize'}
                        </Button>
                      </div>
                    </div>
                    <div style={{ marginBottom: '16px' }}>
                      <Label htmlFor="policy-details" style={{ fontSize: '14px', fontWeight: 600, color: '#003366', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', display: 'block', marginBottom: '8px' }}>
                        Policy Summary
                      </Label>
                      <Textarea
                        id="policy-details"
