...
export function ProductTypeSelector({
      ...
      try {
        ...
        if (!response.ok) {
          ...
          try {
            ...
            console.warn(
              "⚠️ ProductTypeSelector: API returned error (non-critical):",
              errorData,
            );
            errorMessage = errorData.error || errorMessage;
          } catch (_e) {
            console.warn(
              "⚠️ ProductTypeSelector: Could not parse error response (non-critical)",
            );
          }
          console.warn(
            "⚠️ ProductTypeSelector:",
            errorMessage,
        ...
        logger.error("❌ ProductTypeSelector: Error:", err as Error, {
  ...
  if (error) {
    return (
      <div className="space-y-2">
        <Label htmlFor="product-type-manual" className="text-gray-900">
          Product Type
        </Label>
        <Input
  ...
  return (
    <div className="space-y-2">
      {showCustomInput ? (
        <>
          <Label htmlFor="custom-product-type" className="text-gray-900">
            Product Type
          </Label>
        ...
        <>
          <Label htmlFor="product-type" className="text-gray-900">
            Product Type
          </Label>
