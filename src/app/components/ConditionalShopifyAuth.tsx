...
function ConditionalShopifyAuthContent({
  children,
}: {
  ...
  const _isUuidStoreRoute = pathname?.startsWith("/stores/");
  ...
  const shouldUseShopifyAuth =
    hasShopQueryParam && !isAuthPage && !isCoachPage && !isBhbPage;
...
export function ConditionalShopifyAuth({
  children,
}: {
  children: React.ReactNode;
}) {
