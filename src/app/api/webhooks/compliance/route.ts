...
export async function POST(request: NextRequest) {
  ...
  let validation: { valid: boolean; body?: string; error?: string } = { valid: false, error: 'Unknown error' };

  try {
    // Dynamic import webhook validation to avoid issues during build
    const { validateWebhook, extractWebhookMetadata: extractMeta } = await import('@/lib/middleware/webhook-validation');
    ...
    if (!validation.valid) {
      ...
      console.error(`[GDPR] Compliance webhook validation failed: ${validation.error}`);
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    // Re-import for metadata extraction
    const { extractWebhookMetadata } = await import('@/lib/middleware/webhook-validation');
...
async function handleCustomerDataRequest(
  body: string,
  metadata: { topic: string | null; shopDomain: string | null; apiVersion: string | null; webhookId: string | null; triggeredAt: string | null }
