...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const _clientSecret = process.env.SHOPIFY_API_SECRET;
    ...
    try {
      ...
      signatureError =
        "Signature verification skipped (will be checked during Token Exchange)";
    } catch (err) {
      signatureError =
    ...
    const diagnostics = {
      ...
      payload: {
        ...payload,
        exp_human: payload.exp
          ? new Date(payload.exp * 1000).toISOString()
          : null,
        nbf_human: payload.nbf
          ? new Date(payload.nbf * 1000).toISOString()
          : null,
        iat_human: payload.iat
    ...
    // Identify issues
    if (isExpired) {
      diagnostics.tokenExchange.issues.push(
        "❌ Token is expired (60-second lifetime)",
      );
    }
    if (notYetValid) {
    ...
    if (
      !payload.aud ||
      payload.aud !== process.env.NEXT_PUBLIC_SHOPIFY_API_KEY
    ) {
      diagnostics.tokenExchange.issues.push(
        `❌ Audience mismatch - Expected: ${process.env.NEXT_PUBLIC_SHOPIFY_API_KEY}, Got: ${payload.aud}`,
      );
    }
    if (!shop) {
      diagnostics.tokenExchange.issues.push(
        "❌ Could not extract shop domain from token",
      );
    }

    if (diagnostics.tokenExchange.issues.length === 0) {
      diagnostics.tokenExchange.issues.push(
        "✅ Token appears valid for exchange",
      );
    }
