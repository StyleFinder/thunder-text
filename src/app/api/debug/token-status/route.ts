...
export async function GET(request: NextRequest) {
  const guardResponse = guardDebugRoute("/api/debug/token-status");
  ...
  try {
    ...
    // SECURITY: Auth bypass check removed - this pattern should not exist in any environment
    const envStatus = {
    ...
    return NextResponse.json({
      ...
      tokenStatus: {
        ...
        tokenPreview: tokenResult.accessToken
          ? tokenResult.accessToken.substring(0, 15) + "..."
          : null,
      },
