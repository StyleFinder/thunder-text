...
export const DELETE = requireAuth('user')(async (
  ...
  try {
    ...
    if (hardDelete) {
      ...
      if (storageError) {
        logger.error('Error deleting file from storage', new Error(storageError.message), {
          component: 'best-practices-id-api',
      ...
      if (deleteError) {
        logger.error('Error deleting best practice', new Error(deleteError.message), {
          component: 'best-practices-id-api',
          operation: 'DELETE-hardDelete',
          id,
      ...
      if (error) {
        logger.error('Error deactivating best practice', new Error(error.message), {
          component: 'best-practices-id-api',
