...
export const GET = requireAuth("user")(async () => {
  try {
    ...
    if (error) {
      logger.error("Error fetching best practices:", error as Error, {
    ...
    logger.error("Error in best practices API:", error as Error, {
      component: "best-practices",
    });
    return NextResponse.json(
  ...
  try {
    ...
    if (!fileType) {
      return NextResponse.json(
        {
          error:
            "Unsupported file type. Allowed: PDF, Audio (MP3, WAV, M4A), Images (JPEG, PNG, WebP)",
        },
        { status: 400 },
      );
    }

    // SECURITY: Sanitize path segments to prevent path traversal attacks
    // User-controlled values (platform, category, file.name) could contain "../"
    const sanitizedPlatform = sanitizeStoragePathSegment(platform);
    const sanitizedCategory = sanitizeStoragePathSegment(category);
    const sanitizedFileName = sanitizeStoragePathSegment(
      file.name.replace(/[^a-zA-Z0-9._-]/g, "_"),
    );

    // Validate sanitized values are not empty
    if (!sanitizedPlatform || !sanitizedCategory || !sanitizedFileName) {
      return NextResponse.json(
        {
          error:
            "Invalid path components. Platform, category, and filename must contain valid characters.",
        },
        { status: 400 },
      );
    }
    ...
    const filePath = `${sanitizedPlatform}/${sanitizedCategory}/${fileName}`;
    ...
    if (uploadError) {
      logger.error("Error uploading file:", uploadError as Error, {
        component: "best-practices",
      });
      return NextResponse.json(
    ...
    const {
      data: { publicUrl },
    } = supabaseAdmin.storage.from("best-practices").getPublicUrl(filePath);
    ...
    if (dbError) {
      logger.error("Error inserting best practice:", dbError as Error, {
        component: "best-practices",
      });
      // Clean up uploaded file
      await supabaseAdmin.storage.from("best-practices").remove([filePath]);
    ...
    logger.error("Error in best practices POST:", error as Error, {
      component: "best-practices",
    });
    return NextResponse.json(
      { error: "Internal server error" },
