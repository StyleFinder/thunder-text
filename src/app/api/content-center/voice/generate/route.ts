...
} from "@/lib/services/voice-profile-generator";
...
export async function POST(
  request: NextRequest,
): Promise<NextResponse<ApiResponse<GenerateVoiceProfileResponse>>> {
  ...
  try {
    ...
    const rateLimitCheck = await withRateLimit(RATE_LIMITS.VOICE_GENERATION)(
      request,
      userId,
    );
    if (rateLimitCheck)
      return rateLimitCheck as NextResponse<
        ApiResponse<GenerateVoiceProfileResponse>
      >;
    ...
    const { profileText, tokensUsed, generationTimeMs } =
      await generateVoiceProfile(samples as ContentSample[]);

    // Validate profile quality
    const validation = validateVoiceProfile(profileText);
    if (!validation.valid) {
      logger.error(
        `Generated profile failed quality validation: ${JSON.stringify(validation.issues)}`,
        undefined,
        { component: "generate" },
      );
    ...
    if (profileError) {
      logger.error("Error creating voice profile:", profileError as Error, {
        component: "generate",
      });
      return NextResponse.json(
    ...
    logger.error(
      "Error in POST /api/content-center/voice/generate:",
      error as Error,
      { component: "generate" },
    );
