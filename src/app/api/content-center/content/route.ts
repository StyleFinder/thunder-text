...
export async function GET(
  request: NextRequest,
): Promise<NextResponse<ApiResponse<ListContentResponse>>> {
  try {
    ...
    if (rateLimitCheck)
      return rateLimitCheck as NextResponse<ApiResponse<ListContentResponse>>;
    ...
    if (search) {
      query = query.or(
        `topic.ilike.%${search}%,generated_text.ilike.%${search}%`,
      );
    }
    ...
    logger.error("Error in GET /api/content-center/content:", error as Error, {
      component: "content",
    });
    return NextResponse.json(
