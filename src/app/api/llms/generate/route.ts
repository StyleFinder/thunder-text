...
async function fetchAllProducts(
  ...
  while (hasNextPage) {
    const apiResponse: ProductsResponse = await client.request<ProductsResponse>(query, {
    ...
    allProducts.push(...apiResponse.products.edges.map((edge: ProductEdge) => edge.node));
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { products, shopInfo } = await fetchAllProducts(fullShop, accessToken);
