...
export default function WelcomeBHBPage() {
  ...
  const saveStoreProfile = async (): Promise<boolean> => {
    ...
    logger.info("[WelcomeBHB] saveStoreProfile called", {
      component: "welcomebhb",
      hasShop: !!shop,
      hasUserId: !!userId,
      userId: userId || "NONE",
    ...
    if (!shop && !userId) {
      logger.error(
        "[WelcomeBHB] CRITICAL: No shop or userId available to save profile",
        undefined,
        {
          component: "welcomebhb",
          sessionStatus,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error("[WelcomeBHB] Failed to save store profile", undefined, {
          component: "welcomebhb",
      ...
      logger.info("[WelcomeBHB] Store profile saved successfully", {
        component: "welcomebhb",
        shop,
        userId,
      });
      ...
      logger.error("[WelcomeBHB] Error saving store profile:", error as Error, {
        component: "welcomebhb",
      });
  ...
  const handleConnectFacebook = () => {
    ...
    window.location.href = `/api/facebook/oauth/authorize?shop=${shop}&return_to=welcomebhb`;
  };

  const handleConnectGoogle = () => {
    const shop = searchParams.get("shop");
    if (shop) {
      window.location.href = `/api/google/oauth/authorize?shop=${shop}&return_to=/welcomebhb`;
    }
  };
  ...
  const handleGoToDashboard = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        logger.error(
          "[WelcomeBHB] Failed to mark onboarding complete",
          undefined,
          {
            component: "welcomebhb",
            status: response.status,
        ...
        logger.info("[WelcomeBHB] Onboarding marked complete successfully", {
          component: "welcomebhb",
          shop: shop || "standalone",
          userId: session?.user?.id || "none",
        });
      ...
      logger.error(
        "[WelcomeBHB] Error marking onboarding complete:",
        error as Error,
        {
          component: "welcomebhb",
          shop: shop || "standalone",
