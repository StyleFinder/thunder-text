...
export default function ImageGenerationPage() {
  ...
  const { shopId, shopDomain, shop, isLoading: shopLoading } = useShop();

  // State
  const [referenceImage, setReferenceImage] = useState<string | null>(null);
  const [openaiModel, setOpenaiModel] = useState<OpenAIImageModel>('gpt-image-1');
  ...
  const handleDownload = useCallback((format: 'png' | 'jpg' | 'webp' = 'png') => {
  ...
  const handleSaveToLibrary = useCallback(async (image?: GeneratedImage): Promise<boolean> => {
    ...
    const conversationId = imageToSave.conversationId || `save-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
  ...
  const handleAddToProduct = useCallback(async (productId: string) => {
    if (!currentImage) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: colors.background }}>
        <div className="flex flex-col items-center gap-4">
          <Loader2 className="w-8 h-8 animate-spin" style={{ color: colors.smartBlue }} />
          <p className="text-sm" style={{ color: colors.grayText }}>Loading...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: colors.background }}>
      {/* Header */}
      <div className="border-b" style={{ backgroundColor: colors.white, borderColor: colors.border }}>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{ background: `linear-gradient(135deg, ${colors.smartBlue} 0%, ${colors.berryLipstick} 100%)` }}
                >
                  <Sparkles className="w-5 h-5 text-white" />
                </div>
                <div>
                  <h1 className="text-lg font-semibold" style={{ color: colors.oxfordNavy }}>
                    Image Generation
                  </h1>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="flex items-center gap-2">
                    <Library className="w-5 h-5" style={{ color: colors.smartBlue }} />
                    Image Library
                  </CardTitle>
                  <CardDescription>
            ...
            <CardContent>
              {isLoadingLibrary ? (
                <div className="flex items-center justify-center py-12">
                  <Loader2 className="w-6 h-6 animate-spin" style={{ color: colors.smartBlue }} />
                </div>
              ) : library.length === 0 ? (
                <div className="text-center py-12">
                  <ImageIcon className="w-12 h-12 mx-auto mb-4" style={{ color: colors.grayText }} />
      ...
      <ExportDialog
        ...
        onSaveToLibrary={async () => { await handleSaveToLibrary(); }}
      />
      ...
      <AlertDialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => deleteConfirm && handleDeleteFromLibrary(deleteConfirm)}
