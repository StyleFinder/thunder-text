...
export const GOOGLE_CATEGORY_MAPPING: Record<string, string> = {
  ...
  "Clothing > Shirts":
    "Apparel & Accessories > Clothing > Tops > Shirts & Tops",
  "Clothing > T-Shirts":
    "Apparel & Accessories > Clothing > Tops > Shirts & Tops",
...
export function generateGoogleProductMetafields(
  ...
  const googleCategory =
    GOOGLE_CATEGORY_MAPPING[category] || "Apparel & Accessories";
  metafields.push({
    namespace: "google",
  ...
  const gender =
    inferGender(category, productType, description, keywords) ||
  ...
  const ageGroup =
    inferAgeGroup(category, productType, description) ||
    additionalData?.age_group;
  if (ageGroup) {
    metafields.push({
      namespace: "google",
      key: "age_group",
  ...
  if (
    additionalData?.product_highlights &&
    additionalData.product_highlights.length > 0
  ) {
    metafields.push({
      namespace: "google",
...
function inferGender(
  ...
  const femaleMatches = femaleKeywords.filter((keyword) =>
  ...
  const maleMatches = maleKeywords.filter((keyword) =>
    allText.includes(keyword),
  );
  ...
  const unisexMatches = unisexKeywords.filter((keyword) =>
    allText.includes(keyword),
  );
...
function inferAgeGroup(
  category: string,
  productType?: string,
  description?: string,
): string | null {
  const text =
    `${category} ${productType || ""} ${description || ""}`.toLowerCase();
...
function inferMaterial(description?: string): string | null {
  ...
  const fabricMatch = text.match(
    /(\d+%?\s*(?:cotton|polyester|wool|silk|linen|spandex|elastane|nylon|acrylic|rayon|viscose|modal|bamboo|hemp))/i,
  );
  if (fabricMatch) {
...
export function validateGoogleMetafields(
  ...
  const hasCategory = googleMetafields.some(
  ...
  if (!hasGender) {
    warnings.push(
      "gender is recommended for better Google Shopping performance",
    );
  }

  if (!hasAgeGroup) {
    warnings.push(
      "age_group is recommended for better Google Shopping performance",
    );
  }
