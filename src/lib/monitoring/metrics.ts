...
async function getOverviewMetrics(
  days: number,
): Promise<MonitoringMetrics["overview"]> {
  try {
    // First try the rollup-based RPC
    const { data } = await supabaseAdmin.rpc("get_dashboard_summary", {
      p_days: days,
    });
    ...
    const activeStores = new Set(logs.map((r) => r.shop_id).filter(Boolean))
      .size;
...
async function getCostBreakdown(
  days: number,
): Promise<MonitoringMetrics["costBreakdown"]> {
  try {
    ...
    if (rollups && rollups.length > 0) {
      for (const row of rollups) {
        ...
        byOperation[row.operation_type] =
          (byOperation[row.operation_type] || 0) + cost;
...
async function getUsageMetrics(
  _days: number,
): Promise<MonitoringMetrics["usage"]> {
  try {
    ...
    if (weekRollups && weekRollups.length > 0) {
      for (const row of weekRollups) {
        ...
        byOperation[row.operation_type] =
          (byOperation[row.operation_type] || 0) + row.request_count;
      }
      ...
      if (weekLogs) {
        ...
        for (const row of weekLogs) {
          byOperation[row.operation_type] =
            (byOperation[row.operation_type] || 0) + 1;
        }
      }
...
async function getPerformanceMetrics(
  ...
  try {
    ...
    const { data: rollups } = await supabaseAdmin
      .from("daily_usage_rollups")
      .select(
        "request_count, success_count, error_count, timeout_count, avg_latency_ms, p95_latency_ms",
      )
      .gte("date", sinceDate);
    ...
    if (rollups && rollups.length > 0) {
      ...
      return {
        avgLatencyMs:
          totalRequests > 0 ? Math.round(latencySum / totalRequests) : 0,
        p95LatencyMs: maxP95,
        successRate:
          totalRequests > 0
        ...
        timeoutRate:
          totalRequests > 0
            ? Math.round((totalTimeouts / totalRequests) * 1000) / 10
            : 0,
      };
    }
    ...
    const avgLatencyMs =
      logs.reduce((sum, r) => sum + (r.latency_ms || 0), 0) / totalRequests;
...
async function getTopStores(
  days: number,
): Promise<MonitoringMetrics["topStores"]> {
  try {
    ...
    if (
      data &&
      data.length > 0 &&
      data.some((r: { total_generations: number }) => r.total_generations > 0)
    ) {
      return data.map(
        (row: {
          shop_id: string;
    ...
    const results = shops.map((shop) => {
      ...
      const planLimit =
        shop.plan === "pro" ? 5000 : shop.plan === "starter" ? 2000 : 30;
      return {
        shopId: shop.id,
...
export async function getRealTimeStats(): Promise<{
  ...
  try {
    ...
    const avgLatency =
      recentLogs.reduce((sum, r) => sum + (r.latency_ms || 0), 0) /
      recentLogs.length;
...
const SERVICE_MATCHERS: Record<string, ServiceMatcher> = {
  ...
  imageGeneration: {
    ...
    matchFn: (log) =>
      log.operation_type === "image_generation" || log.model === "gpt-image-1",
  },
  productDescriptions: {
  ...
  adGeneration: {
    ...
    matchFn: (log) =>
      log.operation_type === "ad_generation" ||
      log.operation_type === "content_generation",
  },
};
...
export async function getApiHealthMetrics(): Promise<ApiHealthMetrics> {
  try {
    ...
    const totalErrors = allLogs.filter(
    ...
    const overallLatency =
      totalRequests > 0
        ? allLogs.reduce((sum, log) => sum + (log.latency_ms || 0), 0) /
          totalRequests
        : 0;

    const lastAnyRequestLog = (dayLogs || [])[0];
    const lastAnyRequest = lastAnyRequestLog?.created_at || null;
    const lastAnyRequestAge = lastAnyRequest
      ? Date.now() - new Date(lastAnyRequest).getTime()
      : null;
    ...
    for (const [, serviceMatcher] of Object.entries(SERVICE_MATCHERS)) {
      ...
      const serviceLogs = (recentLogs || []).filter((log) =>
        serviceMatcher.matchFn(log),
      );

      // Get last request from day logs
      const lastServiceLog = (dayLogs || []).find((log) =>
        serviceMatcher.matchFn(log),
      );
      ...
      const avgLatencyMs =
        requestCount > 0
          ? serviceLogs.reduce((sum, log) => sum + (log.latency_ms || 0), 0) /
            requestCount
          : 0;

      const lastRequest = lastServiceLog?.created_at || null;
      const lastRequestAge = lastRequest
        ? Date.now() - new Date(lastRequest).getTime()
        : null;
