 ...
 */
function removeAIArtifacts(content: string): {
  content: string;
  found: string[];
} {
  const artifacts = [
    ...
    {
      pattern: /i cannot (provide|create|generate|write)/gi,
      name: "AI limitation",
    },
    { pattern: /i apologize,?\s*/gi, name: "AI apology" },
    {
      pattern: /i don't have (access|the ability|information)/gi,
      name: "AI limitation",
    },
 ...
 */
function formatMarkdown(content: string, _contentType: ContentType): string {
 ...
 */
function formatHTML(content: string, _contentType: ContentType): string {
  let formatted = content;
...
export function calculateSEOScore(
  ...
  if (targetKeywords && targetKeywords.length > 0) {
    ...
    const keywordDensity =
      targetKeywords.filter((kw) => lowerContent.includes(kw.toLowerCase()))
