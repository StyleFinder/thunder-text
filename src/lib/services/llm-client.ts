...
async function executeCompletion(
  ...
  options: Omit<
    ...
    "enableQueue" | "queuePriority" | "queueTimeoutMs"
  >,
  retryOptions: RetryOptions,
): Promise<LLMCompletionResult> {
  const {
  ...
  if (!canRequest(primaryProvider)) {
    ...
    logger.warn(
      `${primaryProvider} circuit breaker is OPEN, attempting fallback`,
      {
        component: "llm-client",
    ...
    if (
      disableFallback ||
      (fallbackProvider === "anthropic" && !anthropicConfigured)
    ) {
      throw new LLMError(
  ...
  try {
    ...
    logger.info(
      `${primaryProvider} failed, falling back to ${fallbackProvider}`,
      {
        component: "llm-client",
...
async function callWithProvider(
  ...
  options: Omit<
    LLMCompletionOptions,
    "model" | "disableFallback" | "preferredProvider"
  >,
  retryOptions: RetryOptions,
  usedFallback: boolean,
): Promise<LLMCompletionResult> {
...
export function getLLMSystemHealth(): {
  ...
  const availableProviders =
    (providers.openai.available ? 1 : 0) +
    (providers.anthropic.available ? 1 : 0);
  const healthyQueues =
    (isQueueHealthy("openai") ? 1 : 0) + (isQueueHealthy("anthropic") ? 1 : 0);
