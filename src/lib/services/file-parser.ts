...
export async function parseFile(file: File): Promise<ParseResult> {
  ...
  if (fileName.endsWith(".txt")) {
    ...
    return {
      ...
      wordCount: 0,
      error:
        "Microsoft Word files (.doc, .docx) parsing coming soon. Please use .txt or .pdf for now, or copy/paste your content.",
    };
    ...
    return {
      ...
      wordCount: 0,
      error:
        "PDF parsing coming soon. Please use .txt for now, or copy/paste your content.",
    };
  } else {
    return {
      text: "",
 ...
 */
export async function parseFileServerSide(
  _fileBuffer: Buffer,
  _mimeType: string,
): Promise<ParseResult> {
