...
import { callChatCompletion, truncateToTokenLimit, estimateTokenCount } from './openai-client'
...
export async function generateVoiceProfile(
  ...
  if (samples.length < 3) {
    throw new Error(`Insufficient samples: need at least 3, got ${samples.length}`)
  }
  ...
  const promptTokens = estimateTokenCount(VOICE_PROFILE_SYSTEM_PROMPT + samplesText)
...
function prepareSamplesForAnalysis(samples: ContentSample[]): string {
  ...
  let samplesText = samples.map((sample, index) => {
 ...
 */
export function validateVoiceProfile(profileText: string): { valid: boolean; issues: string[] } {
  const issues: string[] = []
