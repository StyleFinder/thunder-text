...
export async function generateContent(
  ...
  if (!validation.valid) {
    throw new Error(
      validation.error || "Invalid content generation parameters",
    );
  }
...
function calculateTemperature(
  ...
  const finalTemp = Math.max(
    0.3,
    Math.min(0.9, baseTemp + intensityAdjustment),
  );
...
export function validateGeneratedContent(
  ...
  const actualWordCount = countWords(content);
  const deviation =
    Math.abs(actualWordCount - params.wordCount) / params.wordCount;
...
export async function batchGenerateContent(
  ...
  for (const params of paramsArray) {
    try {
      ...
      logger.error("Batch generation error:", error as Error, {
