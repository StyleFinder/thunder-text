...
export type IntegrationProvider = 'shopify' | 'meta' | 'google' | 'tiktok' | 'pinterest' | 'klaviyo' | 'mailchimp';
export type ProductTier = 'ace' | 'thunder' | 'zeus' | 'bundle_all' | 'coaching_only';
...
export class IntegrationService {
     ...
     */
    async getAccessToken(shopId: string, provider: IntegrationProvider, requiredProduct?: ProductTier): Promise<string | null> {
        // 1. Check Subscription (The Gatekeeper)
        if (requiredProduct) {
            const hasAccess = await this.checkSubscription(shopId, requiredProduct);
            if (!hasAccess) {
                console.warn(`Shop ${shopId} attempted to access ${provider} without active subscription to ${requiredProduct}`);
                return null;
            }
        }
        ...
        if (error || !data) {
            logger.error(`Failed to fetch ${provider} token for shop ${shopId}:`, error as Error, { component: 'integration-service' });
            return null;
        }
     ...
     */
    async saveIntegration(shopId: string, provider: IntegrationProvider, data: IntegrationData): Promise<boolean> {
        ...
        if (error) {
            logger.error(`Failed to save ${provider} integration for shop ${shopId}:`, error as Error, { component: 'integration-service' });
            return false;
        }
     ...
     */
    async checkSubscription(shopId: string, product: ProductTier): Promise<boolean> {
        ...
        if (error) {
            logger.error(`Error checking subscription for shop ${shopId}:`, error as Error, { component: 'integration-service' });
            return false;
        }
     ...
     */
    async disconnectIntegration(shopId: string, provider: IntegrationProvider): Promise<boolean> {
        const { error } = await this.supabase
            .from('integrations')
