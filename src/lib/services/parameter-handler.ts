 ...
 */
export const WORD_COUNT_RANGES: Record<ContentType, { min: number; max: number; recommended: number }> = {
 ...
 */
export function validateWordCountForType(contentType: ContentType, wordCount: number): {
  valid: boolean
  message?: string
} {
...
export function normalizeParametersForType(
  ...
  const normalizedWordCount = Math.max(range.min, Math.min(range.max, wordCount))
 ...
 */
export function estimateGenerationTime(contentType: ContentType, wordCount: number): number {
 ...
 */
export function estimateCost(contentType: ContentType, wordCount: number): number {
 ...
 */
export function parseParametersFromString(paramString: string): Partial<ParameterPreset> | null {
  try {
