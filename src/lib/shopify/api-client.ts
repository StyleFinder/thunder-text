...
export async function getAuthToken(
  shop?: string,
): Promise<{ token: string | null; type: "session" | "oauth" | null }> {
  ...
  if (window.getShopifySessionToken) {
    try {
      ...
      logger.debug("App Bridge not available, trying external auth", {
  ...
  if (shop) {
    try {
      const response = await fetch(
        `/api/shopify/token?shop=${encodeURIComponent(shop)}`,
      );
      if (response.ok) {
      ...
      logger.error("Failed to get OAuth token:", error as Error, {
        component: "api-client",
      });
    }
  }
    ...
    if (authType === "oauth" && shopDomain) {
      try {
        const refreshResponse = await fetch(
          `/api/auth/refresh?shop=${encodeURIComponent(shopDomain)}`,
          {
            method: "POST",
          },
        );
        ...
        logger.error("Failed to refresh OAuth token:", error as Error, {
          component: "api-client",
        });
      }
    } else {
...
export async function fetchProducts(params: URLSearchParams) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 && result.requiresAuth) {
      logger.error(
        "⚠️ Authentication failed. App may need to be reinstalled.",
        undefined,
        { component: "api-client" },
      );
...
export async function enhanceProduct(
  ...
  if (!productId || invalidProductIds.includes(productId.toLowerCase())) {
    logger.error(
      `Invalid productId in enhanceProduct: ${productId}`,
...
export async function getProductPrepopulation(productId: string, shop: string) {
  ...
  if (!productId || invalidProductIds.includes(productId.toLowerCase())) {
    logger.error(
      `Invalid productId in getProductPrepopulation: ${productId}`,
      new Error(`Invalid product ID: ${productId}`),
      { component: "api-client" },
    );
    throw new Error(`Invalid product ID: "${productId}"`);
