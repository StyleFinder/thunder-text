...
[
    ...
    try {
      ...
      return variants.map((variant, index) => {
        const scoreData = scores.find(
          (s: { variant_index: number; score: number; critique: string }) =>
            s.variant_index === index,
        );
        return {
          ...variant,
      ...
      logger.error("Failed to parse Analyst Agent response:", error as Error, {
        component: "agent",
      });
   ...
   */
  private buildBrandVoiceCriteria(
    ...
    if (brandVoice.preferredTerms.length > 0) {
      parts.push(
        `- Should use: ${brandVoice.preferredTerms.slice(0, 5).join(", ")}`,
      );
    }
    if (brandVoice.avoidedTerms.length > 0) {
      parts.push(
        `- Should NOT use: ${brandVoice.avoidedTerms.slice(0, 5).join(", ")}`,
      );
    }
    if (brandVoice.coreValues.length > 0) {
      parts.push(
        `- Should reflect values: ${brandVoice.coreValues.join(", ")}`,
      );
    }
