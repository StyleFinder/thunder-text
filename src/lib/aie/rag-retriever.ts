...
async function retrieveBestPractices(params: {
  ...
  if (error) {
    console.error('Error retrieving best practices:', error);
...
async function retrieveAdExamples(params: {
  ...
  if (error) {
    console.error('Error retrieving ad examples:', error);
  ...
  return data.map((row: any) => ({
    ...
    tone: row.tone,
    performance_tag: row.performance_tag,
...
export async function logRAGRetrieval(params: {
  ...
  try {
    ...
    console.error('Error logging RAG retrieval:', error);
