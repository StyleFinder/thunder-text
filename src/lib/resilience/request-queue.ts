 ...
 */
function insertByPriority<T>(queue: QueuedRequest<unknown>[], request: QueuedRequest<T>): void {
...
export async function enqueue<T>(
  ...
  if (priority === QueuePriority.CRITICAL && queue.activeCount < config.maxConcurrent) {
    queue.activeCount++
    try {
 ...
 */
export function getAllQueueStatuses(): Record<string, ReturnType<typeof getQueueStatus>> {
...
export function isQueueHealthy(serviceName: string): boolean {
  ...
  if (status.totalProcessed > 10) {
    const rejectionRate = status.totalRejected / (status.totalProcessed + status.totalRejected)
 ...
 */
export function estimateWaitTime(serviceName: string, priority: QueuePriority = QueuePriority.NORMAL): number {
  const queue = getQueue(serviceName)
  const config = getConfig(serviceName)
