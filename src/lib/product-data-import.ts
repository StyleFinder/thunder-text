...
export async function importProductData(
  ...
  try {
    const response = await shopifyGraphQL<ProductQueryResponse>(
      PRODUCT_QUERY,
      {
        id: productId,
      },
    ...
    logger.error("Error importing product data:", error as Error, {
      component: "product-data-import",
    });
...
export function detectProductCategory(productData: ProductData): string {
  ...
  const searchText = [productType, title, ...tags, ...collections]
    .join(" ")
    .toLowerCase();
...
export function analyzeExistingDescription(description: string): {
  ...
  const hasBulletPoints =
    /[•\-\*]/.test(description) || description.includes("<li>");
  ...
  if (!hasDescription) {
    ...
    suggestions.push(
      "Current description is long - consider summarizing key points",
    );
  }
  ...
  if (!hasKeywords && hasDescription) {
    suggestions.push(
      "Description could benefit from more descriptive keywords",
    );
  }
