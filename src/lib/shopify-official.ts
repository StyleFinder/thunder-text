...
import type {
  ...
  ShopifyVariantInput,
  ShopifyMediaUploadTarget as _ShopifyMediaUploadTarget,
  ShopifyMediaImage as _ShopifyMediaImage,
  ShopifyProductCreateMediaResponse as _ShopifyProductCreateMediaResponse,
} from "@/types/shopify";
...
export class ShopifyOfficialAPI {
  ...
  async createProduct(productData: ShopifyProductInput) {
    try {
      ...
      logger.error(
        "Error creating product with official Admin API client",
        error as Error,
        {
          component: "shopify-official",
    ...
    try {
      ...
      if (typeof imageData === "string") {
        ...
        if (imageData.data && typeof imageData.data === "string") {
          ...
          throw new Error(
            `Invalid imageData object format - available properties: ${Object.keys(imageData).join(", ")}`,
          );
        }
        ...
        if (!matches) {
          throw new Error(
            `Invalid base64 data URL format in object property: ${dataString.substring(0, 50)}...`,
          );
        }
        mimeType = matches[1];
        base64Data = matches[2];
        ...
        throw new Error(
          "Invalid imageData format - must be string or object with data property",
        );
      }

      if (!mimeType.startsWith("image/")) {
        throw new Error(
          `Invalid media type: ${mimeType}. Only image formats are supported.`,
        );
      }
      ...
      const stagedUploadResponse = await this.client.request(
      ...
      if (stagedUploadData.userErrors?.length > 0) {
        logger.error("Staged upload creation failed", undefined, {
          userErrors: stagedUploadData.userErrors,
          component: "shopify-official",
          operation: "createProductImage",
        });
        throw new Error(
          `Staged upload failed: ${stagedUploadData.userErrors.map((e: { message: string }) => e.message).join(", ")}`,
        );
      }
      ...
      if (buffer.length < 100) {
        throw new Error(
          `Image buffer too small (${buffer.length} bytes) - likely truncated base64 data`,
        );
      }
      ...
      if (uploadResponse.status !== 204 && uploadResponse.status !== 201) {
        ...
        logger.error(
          "File upload failed",
          new Error(`${uploadResponse.status} ${uploadResponse.statusText}`),
          {
            errorText,
            component: "shopify-official",
            operation: "createProductImage",
      ...
      const mediaResponse = await this.client.request(
      ...
      if (mediaData?.mediaUserErrors?.length > 0) {
        logger.error("Media creation failed", undefined, {
          mediaUserErrors: mediaData.mediaUserErrors,
          component: "shopify-official",
          operation: "createProductImage",
        });
        throw new Error(
          `Media creation failed: ${mediaData.mediaUserErrors.map((e: { message: string }) => e.message).join(", ")}`,
        );
      }

      if (!mediaData?.media?.length) {
        logger.error("No media was created", undefined, {
          component: "shopify-official",
          operation: "createProductImage",
        });
        throw new Error(
          "No media was created - productCreateMedia returned empty media array",
        );
      }

      const _createdMedia = mediaData.media[0];
      ...
      logger.error("Error in corrected media upload process", error as Error, {
        component: "shopify-official",
        operation: "createProductImage",
        productId,
      });
  ...
  async createProductOption(
    ...
    try {
      ...
      logger.error(
        "Error creating option with official library",
        error as Error,
        {
          component: "shopify-official",
  ...
  async createProductVariants(
    ...
    try {
      ...
      if (response.data?.productVariantsBulkCreate?.userErrors?.length > 0) {
        logger.error("Variant creation user errors", undefined, {
          userErrors: response.data.productVariantsBulkCreate.userErrors,
          component: "shopify-official",
          operation: "createProductVariants",
          productId,
        });
      }
      ...
      logger.error(
        "Error creating variants with official library",
        error as Error,
        {
          component: "shopify-official",
          operation: "createProductVariants",
  ...
  async createProductMetafield(
    ...
    try {
      ...
      logger.error(
        "Error creating metafield with official library",
        error as Error,
        {
          component: "shopify-official",
  ...
  async createProductVariantMetafield(
    variantId: string,
    metafield: ShopifyMetafieldInput,
  ) {
    try {
      ...
      logger.error(
        "Error creating variant metafield with official library",
        error as Error,
        {
          component: "shopify-official",
          operation: "createProductVariantMetafield",
          variantId,
        },
      );
      throw error;
