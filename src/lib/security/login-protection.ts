...
export function checkLockoutStatus(identifier: string): LockoutStatus {
  ...
  const attemptsInWindow = now - record.lastAttempt < ATTEMPT_WINDOW_MS ? record.attempts : 0
...
export function recordFailedAttempt(identifier: string): LockoutStatus {
  ...
  return {
    ...
    lockoutRemainingSeconds: lockedUntil ? Math.ceil(LOCKOUT_DURATION_MS / 1000) : null
  }
}
