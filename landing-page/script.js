...
function updateROICalculator() {
    const productsCount = parseInt(document.getElementById('products-count').value) || 20;
    const timePerProduct = parseInt(document.getElementById('time-per-product').value) || 15;
    const hourlyRate = parseInt(document.getElementById('hourly-rate').value) || 25;
    ...
    // Update display
    document.getElementById('time-saved').textContent = `${timeSaved.toFixed(1)} hours`;
    document.getElementById('money-saved').textContent = `$${Math.round(moneySaved)}`;
}

// Initialize calculator listeners
document.addEventListener('DOMContentLoaded', () => {
    const calculatorInputs = document.querySelectorAll('#products-count, #time-per-product, #hourly-rate');
...
// Form validation for demo request (if added later)
...
document.querySelectorAll('.btn-primary, .btn-large, .btn-plan').forEach(button => {
